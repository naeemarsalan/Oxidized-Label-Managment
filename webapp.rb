...
module Oxidized
  module API
	...
	location = []
	config_file_local = YAML.load_file('/var/www/html/config')
	count = 0
	for i in config_file_local['groups'] do
                if i[1]['vars']
                        enable = i[1]['vars']['enable']
                else
                        enable = ""
                end

                location.push([i[0],i[1]['username'],i[1]['password'],enable])

                count += 1
        end
	config_file_global = config_file_local
        @array = location
	...
	config_file_local = config_file_global
	if params[:action] == "edit" and params[:ID]
		
		if params[:Password] == ''
			params[:Password] = location[params[:ID].to_i][2]
		end
		
		if params[:Label] == ''
			params[:Label] = location[params[:ID].to_i][0]
		end

		if params[:Username] == ''
                        params[:Username] = location[params[:ID].to_i][1]
                end
		
		if params[:Enable] == ''
                        params[:Enable] = location[params[:ID].to_i][3]
                end
		
                location[params[:ID].to_i] = params[:Label],params[:Username], params[:Password],params[:Enable]
                ...
                config_file_local['groups'] = farray
                File.open("/var/www/html/config", "w") { |file| file.write(config_file_local.to_yaml) }
        else
                if params[:action] == "delete"
                        location.delete_at(params[:ID].to_i)
                        for i in location do
                                farray[i[0]] = {
                                        "username" => i[1],
                                        "password" => i[2],
	...
	location = location_global
        config_file_local = config_file_global
        post_data = params[:data].split(',')
        post_data = post_data.each_slice(4)
        farray = {}
        for a in post_data do
		location.push([a[0].to_s,a[1].to_s,a[2].to_s,a[3].to_s])
	end
	for i in location do
		farray[i[0]] = { "username" => i[1], "password" => i[2], "vars" => { "enable" => i[3] } }
	end
	config_file_local['groups'] = farray
	File.open("/var/www/html/config", "w") { |file| file.write(config_file_local.to_yaml)}
      end
